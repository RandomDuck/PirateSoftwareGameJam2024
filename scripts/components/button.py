...
class Button: # Renders a button with text
  def __init__(self, position, size, color, callback = lambda:print("clicked button")):
    self.color = color #Expected data config: (r,g,b)
  ...
  def render(self, surface):
    ...
    if pygame.mouse.get_pressed()[0] and mouseIsHovering:
      if not self.pressed:
        self.callback()
      self.pressed = True
    elif mouseIsHovering and self.pressed:
      self.pressed = False


class TextButton(Button): # Renders a button with text
  def __init__(self, position, text, size, color, text_color, center, callback = lambda:print("clicked button")):
...
class IconButton(Button): # Renders a button with an icon
  def __init__(self, iconPath, position, size, color, callback = lambda:print("clicked button")):
    super().__init__(position, size, color, callback)
